    ...
    private  String addSubCategory (String SubCategoryName, String categoryName){
        ...
        if (SubCategoryName.equals("")){
            ...
            try {
                ...
                String sqlInsertCompany ="INSERT INTO  `ipass`.`sub-category` (`subcategory_id` ,`sub-category_name` ,`category_name`)VALUES (NULL ,  '"+SubCategoryName+"',  '"+categoryName+"');"; 
                Statement st = con.createStatement();
                int rows = st.executeUpdate(sqlInsertCompany);
                
                if (rows != 1){
                   // out.println("Company not inserted");
                    message = "Sub Category toevoeging geannuleerd.";
                }else {
                    //out.println(companyName+" Company Inserted");
                    message = SubCategoryName+" Sub-Category toegevoegd" ;
                }
                st.close();
                con.close();
            } 
