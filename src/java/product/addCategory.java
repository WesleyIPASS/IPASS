...
public class addCategory extends HttpServlet {
    ...
    private  String addCategory (String categoryName){
        ...
        if (categoryName.equals("")){
            ...
            message ="Vul een categorienaam in";
        }
        else {
            try {
                //out.println("Done!!");
                ...
                String sqlInsertCompany ="INSERT INTO  `ipass`.`category` (`category_id` ,`category_name`)VALUES (NULL ,  '"+categoryName+"');"; 
                Statement st = con.createStatement();
                int rows = st.executeUpdate(sqlInsertCompany);
                
                if (rows != 1){
                   // out.println("Company not inserted");
                    message = "categorie toevoeging mislukt.";
                }else {
                    //out.println(companyName+" Filiaal toegevoegd");
                    message = categoryName+" categorie toegevoegd";
                }
                st.close();
                con.close();
            } 
            catch (SQLSyntaxErrorException ex){
                message = ("Gelieve namen in te vullen zonder aanhalingstekens");
            }
            catch (SQLIntegrityConstraintViolationException ex){
                //out.println("A comany name with the same name exists in your database... Try being specific.");
                message = "Een categorie naam met dezelfde naam bestaat al. Probeer een andere naam";
            }
            catch (SQLException ex) {
                //out.println("There was a problem in Connectiong DB <br/> Exception has occoured "+ex);
                message = "Er was een probleem om verbinding te maken met de database "+ex;
           
            } catch (ClassNotFoundException ex) {
                //out.println("Application Cannot find the Class <br/> Exception has occoured "+ex);
                message = "Applicatie kan de klasse niet vinden "+ex;
            }
        }
        return message;
    }
